...
public interface ContentRepository extends JpaRepository<Content,Long> {
    ...
    @Query("select c from Content c join fetch c.image join fetch c.user")
    ...
    @Query("select c from Content c join fetch c.user join fetch c.image where c.team = :team and c.position = :position ")
    Page<Content> findAllWithNickname(Pageable pageable, @Param("team") Team team, @Param("position") Position position);

    @Query("select c from Content c join fetch c.user join fetch c.image where c.team = :team ")
    Page<Content> findAllWithNicknameByTeam(Pageable pageable, @Param("team") Team team);

    @Query("select c from Content c join fetch c.user join fetch c.image where c.position = :position ")
    Page<Content> findAllWithNicknameByPosition(Pageable pageable, @Param("position") Position position);

    @Query("select c from Content c join fetch c.image where c.id=:id")
